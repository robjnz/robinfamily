<div id="work-card" class="m-4 p-4 w-auto rounded-lg shadow-2xl">
	<div class="p-6">
		<h1 class="text-2xl text-white">Lipes</h1>

		<img
			class="h-32 w-full object-fill"
			src="https://res.cloudinary.com/dzhbfdfa5/image/upload/c_scale,h_200,w_300/v1638859747/webp/database.webp"
			alt="image of a database"
		/>
		<br />
		<p>
			Search the 1897 <br />Jewish Russian census
		</p>
		<br />

		<a class="text-white hover:text-yellow-500" href="https://bit.ly/3bOUFXd" target="_blank">
			Go to The Website
		</a>
	</div>
</div>
